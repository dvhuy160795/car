...
<script>
        ...
        function funcEdit(e,id){  
            ...
            window.location.href = '?control=mngCategory&action=Show&id='+catalogId+'&name='+changeName;
}
        function onCreate(){
        ...
        function funcCreate()
        {
            ...
            window.location.href = '?control=mngCategory&action=Show&AddNameCategory='+name;
        }
</script>
                ...
                <div class="row">
                    <div class="col-md-4">
                        <table class="table table-bordered">
                            ...
                            <form name="input_change_form" method="POST" action="?control=mngCategory&action=Show">
                            <?php $i=1;
                            foreach ($category as $row){  ?>
                            <tr id="<?PHP echo $row['hsx_id'];?>" name="catalog_<?PHP echo $row['hsx_id'];?>" class=" catalogclass">
                               
                                <td class="id_row"><?php echo $i++ ?></td>
                                <td class="name_row"><?php echo $row['hsx_ten']; ?></td>
                                <td class="data_row">
                                    <div class="controls">
                                        <a href="?control=mngCategory&action=Delete&id=<?PHP echo $row['hsx_id'];?>" class="btn btn-danger delete">Xoá</a>
                                        <a onclick="editCatalog(event,<?PHP echo $row['hsx_id'];?>)" class="btn btn-danger ">Sửa</a>
                                    </div>
                                </td>
                            </tr>
                            <?php } ?>
